...
import { reactorStats } from '@/components/tfd/reactor/types';
...
const Reactors: FC<ReactorsProps> = ({ reactors, seo }) => {
  ...
  return (
    <>
      <Header seo={seo} />
      <Container>
        <div className="reactor-data flex flex-col justify-center rounded-xl border-2 border-white bg-slate-900">
          <div className="p-4">
          ...
          <div className="flex flex-row flex-wrap justify-center gap-4 p-4 text-xl">
            {reactorStats.subStats.map(stat => <div key={stat} className="rounded-xl border-2 border-white bg-slate-800 px-5 py-2 text-center">{stat}</div>)}
          </div>
        </div>
      </Container>
      <Footer />
