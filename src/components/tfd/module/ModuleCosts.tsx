...
const ModuleCosts: FC<ModuleCostsProps> = ({ label, kuiperPerLevel }) => {
    ...
    return (
      <div key={currentKuiper} className={['odd:bg-slate-800 even:bg-slate-700', gridClasses].join(' ')}>
        {values.map((value, valueIndex) => {
          ...
          const borderClass = valueIndex % 2 === 1 ? 'border-l-1 border-white px-2' : undefined;
  ...
  const headers = costTypes.map((cost, index) => {
    ...
    return (
      <div key={cost} className="col-span-4 flex flex-col border-l-1 border-white">
        <span className={['pl-2 border-b-1 border-white', labelColor].join(' ')}>{cost}</span>
        <div className="grid grid-cols-2">
          {costSublabels.map((sublabel, sublabelIndex) => (
            <span key={sublabel} className={sublabelIndex === 0 ? 'px-2': undefined}>{sublabel}</span>
          ))}
        </div>
      </div>
    );
  ...
  return (
    ...
    <div className="flex flex-col overflow-clip rounded-lg border-2 border-black bg-slate-900 shadow-lg shadow-black">
      ...
      <div className={['border-t-1 border-white', gridClasses].join(' ')}>
